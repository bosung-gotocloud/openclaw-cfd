...
compute_mesh.py            (Stage 2)
    ...
    ├─ **ViscousLayers(h1=0.0001, layers=5, growth=1.3524, far_faces)** — BOUNDARY LAYERS
...
**snappyMesh / snappyMesh-tip_refine와 동일한 xl 기반 로직을 사용.**
...
경계층 두께는 **계산만 하고 sizing에 사용하지 않음**. 사용자에게 출력만.

``` 
h1 = 0.0001  (첫번째 격자 높이, m)
growth = 1.3524   (성장률)
layers = 5    (경계층 층수)

T = h1 × (growth^layers - 1) / (growth - 1)  (총 두께 - 계산/출력만, ViscousLayers에 전달하지 않음)
```

- `h1`, `growth`, `layers`는 JSON에서 직접 수정 가능
- `T`는 참조용 - mesh sizing에 영향 없음
