...
def run_compute_mesh():
    ...
    derived_rows = [
        ...
        ["surf_size", f"{surf_size:.6f} m", "xl * 0.01 (surface cell size)"],
        ["min_size", f"{min_size:.6f} m", "= T (BL thickness)"],
    ...
    # Define mesh groups (after local size applied)
    ...
    # ViscousLayers: firstLayerHeight=h1, nbLayers=layers, expansionRatio=growth
    ...
    if wake_line_objects:
        print(f"  Applying Netgen local size to {len(wake_line_objects)} wake lines: {tip_wake_refine_size:.6f} m (Step 5: surf_size)")
    ...
    if success:
        ...
        # Export to OpenFOAM case (polyMesh only)
