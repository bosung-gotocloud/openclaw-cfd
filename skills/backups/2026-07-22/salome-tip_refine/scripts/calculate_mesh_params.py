...
Step 3: min_size = surf_size / 4
Step 4: refine_size (refine_local_size) = max_size / 4
Step 5: tip_wake_refine_size = surf_size
...
def run_calculate():
    ...
    # Step 2: surf_size = xl * 0.01
    surf_size = xl * 0.01

    # Step 3: min_size = T (BL thickness)
    min_size = T
    ...
    summary_rows = [
        ...
        ["max_size", f"{max_size:.6f} m", "xl / 5 (coarsest)"],
        ["surf_size", f"{surf_size:.6f} m", "xl * 0.01 (surface)"],
        ["min_size", f"{min_size:.6f} m", "T (BL thickness)"],
