...
|------|------|------|
| **tip_wake_refine_size** | **surf_size** (= xl × 0.005) | Wake line local cell size |
...
Step 2: surf_size  = xl × 0.005    (표면 cell size = 물체 X 길이의 0.5%)
Step 3: min_size   = surf_size / 4  (최소 cell size)
Step 4: refine_size = max_size / 4  (refine box local size)
Step 5: tip_wake_refine_size = surf_size  (tip wake line local size)
```

> `T` (BL total thickness)는 계산/출력만 하고 **sizing에 사용하지 않음**.
...
| T | Total BL thickness | derived (display only — NOT used in sizing) |
| min_size | Minimum mesh size | = surf_size / 4 |
| surf_size | Surface mesh size | = **xl × 0.005** |
| max_size | Max mesh size | = **xl / 5** |
| refine_local_size | Refine box local size | = max_size / 4 |
| tip_wake_refine_size | Wake line local size | = **surf_size** |
...
| `tip_wake_refine_size` | ✗ | = **surf_size** (= xl × 0.005) |
| Mesh sizing logic | T-based (old) | **xl-based** (unified) |
| **addLayersControls** | template 기반 | **firstAndExpansion, nLayers=10, expansionRatio=1.2, maxFaceThicknessRatio=5.0, minFaceWeight=-1, minVolRatio=-1, nBufferCellsNoExtrude=3** |
