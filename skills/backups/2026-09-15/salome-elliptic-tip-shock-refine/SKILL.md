---
name: salome-elliptic-tip-shock-refine
...
- STEP 파일에서 가장 작은 face bbox diagonal 계산
- `min_size > smallest_face_diag` 면 `args['min_size'] = smallest_face_diag`로 자동 조정
...
- STEP 파일에서 가장 작은 face bbox diagonal 계산 → `min_size`보다 작으면 `args['min_size'] = smallest_face_diag`로 자동 조정
