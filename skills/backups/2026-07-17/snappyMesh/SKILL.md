...
>
> **Note (2026-07-12):** snappyMesh 실행 후 `constant/polyMesh/boundary`에서 STL surface patch의 type이 `patch`로 나올 수 있음 (snappyMesh 버그). `snappy_mesh.py`는 `reconstructParMesh` 후 STL surface patch의 type을 `wall`로 강제 변환함.
