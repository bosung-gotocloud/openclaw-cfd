...
def setup_snappy_hex_mesh_case(base_dir, base_name, h1, growth, layers, base_minthickness, totalThickness, template_dir):
    ...
    content = content.replace("BASE_TOTALTHICKNESS", f"{totalThickness}")
...
def run_compute_mesh():
    ...
    case_dir, stl_dir = setup_snappy_hex_mesh_case(base_dir, base_name, h1, growth, layers, base_minthickness, totalThickness, template_dir)
...
""" % (num_procs, num_procs))
        ...
        run_cmd = (
            ...
            f"cd {case_dir} && mpirun --oversubscribe -np {num_procs} snappyHexMesh -parallel > {log_file} 2>&1'"
        )
