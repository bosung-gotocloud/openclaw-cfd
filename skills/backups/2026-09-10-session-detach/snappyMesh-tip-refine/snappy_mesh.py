...
def main():
    ...
    cmd = ("bash -c 'source /opt/OpenFOAM/OpenFOAM-v2512/etc/bashrc 2>/dev/null && "
           "surfaceConvert " + stl_case + " " + obj_case + " -write-format ascii 2>&1' 2>&1 | tee " + obj_log)
    ret = os.system(cmd)
    ...
    h1 = p.get('h1', 0.0001)
    totalThickness = p.get('totalThickness', 0.002)
    growth = 1.2  # fixed
    # addLayers parameters from JSON
    layers = p.get('layers', 10)       # user-settable, default 10
    h1 = p.get('h1', 0.0001)           # firstCellHeight
    growth = p.get('growth', 1.2)       # expansionRatio
    ...
    BASE_MINTHICKNESS = p.get('BASE_MINTHICKNESS', totalThickness * 0.5)
    ...
    content = content.replace('BASE_MINTHICKNESS', f'{BASE_MINTHICKNESS:.8f}')
    ...
    # === 7. Find the STL surface patch name ===
    stl_patch_name = None
    ...
    run_cmd = (
        f"bash -c 'source /opt/OpenFOAM/OpenFOAM-v2512/etc/bashrc && cd {case_dir} && mpirun -np {num_procs} snappyHexMesh -parallel > {log_file} 2>&1'"
    )
    ...
    # === 11b. Fix STL surface patch type to wall ===
    boundary_file = os.path.join(case_dir, 'constant', 'polyMesh', 'boundary')
    ...
    # Create case.foam file (OpenFOAM case marker) — always
