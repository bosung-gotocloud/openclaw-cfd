...
def run_parallel_simplefoam(case_dir, num_procs):
    ...
    cmd = f"bash -c 'source /opt/OpenFOAM/OpenFOAM-v2512/etc/bashrc && cd {case_dir} && mpirun -np {num_procs} --oversubscribe simpleFoam -parallel 2>&1 | tee {case_dir}/log.simpleFoam'"
    ret = os.system(cmd)
    if ret != 0:
