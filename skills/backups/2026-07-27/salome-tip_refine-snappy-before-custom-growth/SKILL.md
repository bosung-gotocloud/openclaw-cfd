...
**Last confirmed:** 2026-07-27 (BL-based sizing: surf_size=4*T, min_size=2*T, tip_wake_refine_size=2*s surf_size; Custom mesh quality)
...
| growth | BL growth rate (snappyHexMesh) | **1.5** |
...
| **NETGEN quality options** | **growth_rate=0.3, segs/edge=1, segs/radius=3, SetOptimize(1)** — custom params disabled — 2026-07-27 |
| **sizing unified** | **layers=10, growth=1.5, min_size=2*T, tip_wake_refine_size=2*surf_size** — 2026-07-27 |
| **Wake lines position** | **After boolean cut** (same as salome-tip_refine) — 2026-07-24 |
| **min_size auto-correction** | **STEP smallest face bbox diagonal 체크. `min_size > smallest_face_diag` 면 `args['min_size'] = smallest_face_diag`로 자동 조정 — 2026-07-26** |
...
### **2026-07-26 - min_size auto-correction 추가**

**compute_mesh.py** — min_size auto-correction 로직 추가 (salome-tip_refine에서 확인):

- STEP 파일에서 가장 작은 face bbox diagonal 계산
- `min_size > smallest_face_diag` 면 `args['min_size'] = smallest_face_diag`로 자동 조정
- **이유**: Netgen은 min_size보다 작은 face를 meshing할 수 없음. LC62-50B의 경우 smallest face diag 0.001686m → min_size 0.005192를 0.001686으로 조정
- **결과**: mesh computation time 단축 + mesh quality 개선 (aspect ratio 2099 → 38.7)
- **Location**: geometry import 후 bbox diagonal 계산 + domain cut 후 min_size 조정
...
**sizing 로직 BL 기반 확정:**
- `h1=0.0001, layers=10, growth=1.5` (snappyHexMesh BL params)
- `T = h1 × (growth^10 - 1) / 0.5` (BL total thickness)
- `surf_size = 4 × T` (surface cell size)
- `min_size = 2 × T` (minimum cell size)
- `tip_wake_refine_size = surf_size × 2`

**Mesh Quality 확정:**
- **fineness=Custom** — mesh density 제어용 (growth_rate=0.3, segs/edge=1, segs/radius=3)
