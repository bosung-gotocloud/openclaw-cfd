...
def run_compute_mesh():
    ...
    derived_rows = [
        ["BL (T)", f"{T:.6f} m", "Total BL thickness (configured via ViscousLayers)"],
    ...
    if tip_wake_lines:
        ...
        derived_rows.append(["tip_refine_size", f"{tip_wake_refine_size:.6f} m", "Wake line local size"])
    ...
    # ViscousLayers: use face list (matches salome scripts/compute_mesh.py)
    print(f"  Configuring viscous layers: {layers} layers, growth {growth}, thickness {T:.6f} m")
