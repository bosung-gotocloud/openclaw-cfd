...
def main():
    ...
    # === Boundary layer ===
    # Default values — user can modify in JSON
    h1 = 0.0001       # firstCellHeight (m)
    growth = 1.2       # expansion ratio
    layers = 10        # number of layers (user-settable)

    # far-field domain box 차원 중 가장 큰 값의 1/50 → coarsest cell size
    # box_dx=10xl, box_dy=5yl, box_dz=10zl (far-field bounding box)
    max_size = max(box_dx, box_dy, box_dz) / 50.0

    # Step 3: surf_size (top-down: target first, then find level)
    target_surf_size = xl * 0.005  # xl의 0.5% = stl x길이의 0.5%
    best_level = 0
    ...
    # Step 4: surf_size = max_size × (0.5^surf_size_level)
    surf_size = max_size * (0.5 ** surf_size_level)

    # Step 5: min_size_level = surf_size_level + 2 (default, user-settable)
    min_size_level = surf_size_level + 2

    # Step 6: min_size = surf_size / 4 (= max_size × (0.5)^min_size_level)
    min_size = surf_size * 0.25  # = surf_size × (1/4)
    ...
    print(f"  h1 = {h1:.6f} m (firstCellHeight)")
    print(f"  growth = {growth} (expansion ratio)")
    print(f"  layers = {layers} (number of layers, user-settable)")
    print(f"  max_size ≈ {max_size:.8f} m (max(box_dx,box_dy,box_dz)/50, far-field coarsest)")
    print(f"  target_surf_size = {target_surf_size:.10f} m (xl × 0.005)")
    print(f"  surf_size_level = {surf_size_level}  →  max_size × (1/2)^{surf_size_level} = {surf_size:.10f} m  (closest to 0.5%xl, diff={best_diff:.10f})")
    print(f"  surf_size = {surf_size:.10f} m")
    print(f"  min_size_level = {min_size_level}  (= surf_size_level + 2)")
    print(f"  min_size = {min_size:.10f} m  (= surf_size / 4)")

    # Step 8: max_level and feature_level
    ...
    print(f"  h1 = {h1:.6f} m (firstCellHeight)")
    print(f"  layers = {layers} (number of layers)")
    print(f"  growth = {growth} (expansion ratio)")
    print(f"  surf_size = {surf_size:.6f} m  (level {surf_size_level})")
    print(f"  min_size = {min_size:.6f} m  (level {min_size_level})")
    ...
    summary_rows = [
        ...
        ["h1", f"{h1:.6f} m", "firstCellHeight (first layer thickness)"],
        ["layers", layers, "number of layers (user-settable, default 10)"],
        ["growth", growth, "expansion ratio (user-settable, default 1.2)"],
        ["min_size", f"{min_size:.8f} m", "Cell size at level {min_size_level} (= surf_size / 4)"],
        ["surf_size", f"{surf_size:.8f} m", "Surface cell size (level {surf_size_level}, = xl × 0.005 closest)"],
        ["max_size", f"{max_size:.8f} m", "level 0 (coarsest)"],
        ["surf_size_level", surf_size_level, "Surface refinement level"],
        ["max_level", max_level, "= min_size_level"],
    ...
    print(f"    2. Review/edit h1, layers, growth if needed")
