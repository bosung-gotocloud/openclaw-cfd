...
Dependencies:
    pip install cadquery numpy Pillow opencv-python trimesh
"""

import argparse, json, os, sys, numpy as np, cv2, tempfile
...
def render_ortho_stl(edges, shape, view, bbox_mm, data, img_size=1100):
    """STL face-filled orthographic view.
    
    STEP shape를 trimesh로 변환 → orthographic projection → face filled (gray) + edges (black).
    외곽선은 black line, 내부는 gray face fill로 덮어서 내부 선이 보이지 않음.
    """
    import trimesh
    ...
    def edges_only_img():
        """Fallback: draw edges only (original behavior)."""
    ...
    # STEP → trimesh (STL temp export → load)
    stl_path = tempfile.mktemp(suffix='.stl')
    shape_exported = False
    try:
        if shape is not None:
            # Workplane has .vals()[0] which is the actual Solid with exportStl
            if hasattr(shape, 'vals') and len(shape.vals()) > 0:
                shape.vals()[0].exportStl(stl_path)
                shape_exported = True
            elif hasattr(shape, 'exportStl'):
                shape.exportStl(stl_path)
                shape_exported = True
    except Exception as e:
        print(f"  [STL export warning] {type(e).__name__}: {e}")
        shape_exported = False

    if not shape_exported:
        # No shape available — edges only fallback
        img = edges_only_img()
    ...
    try:
        ...
        verts = mesh.vertices  # (N, 3) in mm
        faces = mesh.faces  # (M, 3) indices
        n_verts, n_faces = len(verts), len(faces)
    except Exception as e:
        print(f"  [STL load warning] {type(e).__name__}: {e}")
        img = edges_only_img()
        di = {'fit_scale':fit_scale, 'offset_x':off_x, 'offset_y':off_y,
              'h_min':h_min, 'h_max':h_max, 'v_min':v_min, 'v_max':v_max,
              'h_span':h_span, 'v_span':v_span,
    ...
    # Draw filled faces (gray) + edges (black) on white background
    img = np.full((img_size, img_size, 3), 255, dtype=np.uint8)
...
VL_PROMPT = "Find ALL tips, sharp corners, wing tips, tail tips, canard tips — sharp concave points. Return ONLY JSON with double quotes: {\"tips\": [{\"tip_id\": N, \"x\": pixelX, \"y\": pixelY}, ...], \"image_width\": W, \"image_height\": H}"
...
def triangulate(tips_by_view, verts_mm, bbox_mm):
    ...
    # 조건: y, z가 tol(=bbox_diag*0.05) 이내이면 하나로 취급 → x가 제일 큰 것만 남김
    tol = max(bbox_mm['xl'], bbox_mm['yl'], bbox_mm['zl']) * 0.05
    deduped = []
    for t in result:
        placed = False
...
def main():
    ...
    for vn in ['XZ','YZ','XY']:
        # STEP → trimesh (face-filled rendering)
        shape = None
        try:
            shape = cq.importers.importStep(args.step_file)
        except:
            pass
        img, di = render_ortho_stl(edges, shape, vn, bbox_mm, data, img_size=img_size)
            ...
            if tips_vlm_scaled:
                ...
                try:
                    ...
                    for tip in tips_vlm_scaled:
                        ...
                        # 주황색 큰 원: 외경 r=35 선 두께 5, 내경 r=25 채움
    ...
    # Triangulate (tips_by_view: mm, verts_mm: mm, bbox_mm: mm)
    tips_3d = triangulate(tips_by_view, verts_mm, bbox_mm)
