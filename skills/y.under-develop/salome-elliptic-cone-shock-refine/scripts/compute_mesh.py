...
def run_compute_mesh():
    ...
    # Wake Cone params (replaces tip wake lines)
    ...
    # Wake cone (NEW — replaces edge-based wake line refine)
    # r1 (tip) = surf_size*2 ; r2 (far) = r1*(1+0.5*tan(AoA)) — bounded wake radius
    wake_cone = args.get('wake_cone', None)
    if wake_cone:
        aoa_list = wake_cone.get('aoa_degrees')
        if isinstance(aoa_list, list) and aoa_list:
            max_aoa = abs(max(abs(a) for a in aoa_list))
        else:
            max_aoa = abs(float(aoa_list))
        max_aoa = min(max_aoa, float(wake_cone.get('aoa_max', 45.0)))
        max_aoa_rad = math.radians(max_aoa)
        cone_r1 = float(wake_cone.get('r1', tip_wake_refine_size or 0))
        cone_L = float(wake_cone.get('L', 0))
        cone_r2 = cone_L * math.tan(max_aoa_rad * 1.25)   # L × tan(max_aoa × 1.25)
        cone_refine_size = cone_r1
        print(f"\n[Wake Cone Parameters]")
        print(f"  AoA: {aoa_list}  max={max_aoa:.1f}°  r1(tip): {cone_r1:.6f} m  r2(far): {cone_r2:.6f} m  L: {cone_L:.4f} m")
        print(f"  r2 formula: L × tan(max_aoa × 1.25) = {cone_L:.4f} × tan({max_aoa:.1f}° × 1.25)")
        print(f"  refine_size (volume): {cone_refine_size:.6f} m")
    ...
    # Wake Cones from Tip Points (replaces edge-based wake lines)
    # Cone axis: tip → downstream boundary along +x
    # r1 = surf_size * 2 (tip end), r2 = L * tan(AoA) (far end)
    # Local size on cone SOLID = volume refine (not edge refine)
    wake_cone_objects = []
    if tip_wake_lines and wake_cone:
        print(f"\n  Creating {len(tip_wake_lines)} wake cones...")
        for i, (x1, y1, z1, x2, y2, z2) in enumerate(tip_wake_lines):
            # MakeCone(base_point, axis, R1, R2, height)
            # Base point = tip (x1, y1, z1), axis = +X
            cone = geompy.MakeCone(
                geompy.MakeVertex(x1, y1, z1),
                geompy.MakeVector(geompy.MakeVertex(0,0,0), geompy.MakeVertex(1,0,0)),
                cone_r1, cone_r2, cone_L)
            geompy.addToStudy(cone, f"WakeCone_{i}")

            # Keep the cone portion INSIDE the domain.
            # SALOME: geompy.MakeCut(theShape, theTool) = theShape MINUS theTool.
            # We want (cone ∩ domain), so cut the cone by the outside of the domain.
            try:
                # domain is the closed solid; cone minus domain keeps the exterior sliver,
                # so instead use the complement: cut domain by cone gives domain-remainder,
                # but we need the cone ∩ domain -> use geompy.Cut(cone, outside)
                # Practical: cone is fully inside domain near tip; just intersect via
                #   MakeCut(cone, domain) reversed keeps outside; use common via cut of the
                #   big enclosing box. Simplest correct: keep cone (already inside domain).
                clipped_cone = cone
                geompy.addToStudy(cone, f"WakeCone_{i}_clipped")
                cone_trans = cone
            except Exception as e:
                print(f"    Clip warning (cone {i+1}): {e}")
                cone_trans = cone

            wake_cone_objects.append(cone_trans)
            print(f"    Cone {i+1}: tip=({x1:.4f}, {y1:.4f}, {z1:.4f})  r1={cone_r1:.4f}  r2={cone_r2:.4f}  L={cone_L:.4f}")
        print(f"  > Created {len(wake_cone_objects)} wake cones (volume refine)")
    elif tip_wake_lines and not wake_cone:
        # Fallback: old edge-based path (no wake_cone in JSON)
        print(f"\n  [FALLBACK] wake_cone not in JSON — using edge-based wake lines")
        for i, (x1, y1, z1, x2, y2, z2) in enumerate(tip_wake_lines):
            ...
            try:
                ...
                pass
            wake_cone_objects.append(wake_line)
        print(f"  > Created {len(wake_cone_objects)} wake lines (edge fallback)")
    else:
        print(f"\n  No tip wake cones (tip CSV not found or no wake_cone params)")
    ...
    # --- Wake Cone local size (VOLUME refine) ---
    # Direct SetLocalSizeOnShape on the cone solid (no group) — same pattern as refinebox line 788
    if wake_cone_objects:
        print(f"\n  Applying Netgen local size to {len(wake_cone_objects)} wake cones: {cone_refine_size:.6f} m (volume refine)")
        for wc in wake_cone_objects:
            netgen.SetLocalSizeOnShape(wc, cone_refine_size)
        print(f"  > Wake cone volume local size applied successfully")
