...
def load_tip_csv(step_file):
    '''STEP 파일과 동일한 디렉토리에서 tip_points.csv 자동 로드'''
    base = os.path.splitext(step_file)[0]
    csv_path = base + '_tip_points.csv'
...
def load_tip_figure_mesh(mesh):
    '''mesh의 bbox와 tip_points.csv의 좌표 범위가 맞는지 확인 후 로드'''
    b = mesh.bounds
    step_bbox = [tuple(b[0]), tuple(b[1])]
    # 현재 STEP 파일에서 tip CSV 로드 시도
    # callback에서 이미 tip_points가 전달되면 여기서는 로드하지 않음
    return None  # 항상 callback에서 전달받음
...
# ============================================================
    ...
    """Load STEP file via cadquery. Auto-detect unit from geometry size."""
    try:
        shape = cq.importers.importStep(step_file)
        if shape is None or len(shape.vals()) == 0:
            return None
        val = shape.vals()[0]
        # tessellate first to get raw coordinates for auto-detection
        verts, faces = val.tessellate(1e-3)
        verts_arr = np.array([[v.x, v.y, v.z] for v in verts])
        if len(verts_arr) == 0:
            return None
        # Auto-detect: CAD mm coords have diag > 100, m coords have diag <= 100
...
def load_step(step_file):
    ...
    try:
        ...
        if shape is None or len(shape.vals()) == 0:
            return None
        val = shape.vals()[0]
        # isValid() unreliable - CAD may report False for valid geometries
        # tessellate first to get raw coordinates for auto-detection
        verts, faces = val.tessellate(1e-3)
        verts_arr = np.array([[v.x, v.y, v.z] for v in verts])
        if len(verts_arr) == 0:
...
def step_to_stl_mesh(step_file):
    '''STEP 파일을 trimesh STL mesh로 변환. mm → m 자동 감지.'''
    shape = cq.importers.importStep(step_file)
    if shape is None or len(shape.vals()) == 0:
        return None
    val = shape.vals()[0]
    ...
    raw_range = verts_arr.max(axis=0) - verts_arr.min(axis=0)
    diag = np.linalg.norm(raw_range)
    # mm coords have diag > 100, m coords have diag <= 100
    if diag > 100:
        verts_arr = verts_arr / 1000.0
    mesh = trimesh.Trimesh(vertices=verts_arr, faces=faces, process=False)
    return mesh
...
if len(sys.argv) < 2:
    print("Usage: python web-step-viewer.py <step_file>")
    sys.exit(1)

step_file = sys.argv[1]
info = load_step(step_file)
if info is None:
...
# Auto-load tip CSV if exists
_auto_tip = load_tip_csv(step_file)
if _auto_tip:
    _tip_points_store = _auto_tip
    _tip_text_store = f"[{len(_auto_tip)} tips auto-loaded from CSV]" + "\n" + "\n".join([f'tip{j+1}: ({t[0]:8.4f}, {t[1]:8.4f}, {t[2]:8.4f})' for j, t in enumerate(_auto_tip)])
    print(f"  Auto-loaded {len(_auto_tip)} tip points from CSV")
else:
    print(f"  No tip CSV found at {os.path.splitext(step_file)[0]}_tip_points.csv")
...
app.layout = html.Div([
    html.Div([
        dcc.Upload(id='file-upload',
            children=html.Div([html.P("📁 Drag & Drop or click to browse STEP")],
        ...
        html.P(id='file-name', children=f"📄 {os.path.basename(step_file)}", style={"fontSize":14,"fontWeight":"bold"}),
        ...
        dcc.Slider(id='tol-slider', min=0.01, max=10.0, step=0.1, value=5.0,
                    marks={0.01: '0.01', 1.0: '1.0', 5.0: '5.0', 10.0: '10.0'}),
        # Tip list panel
...
def unified_update(x_val, y_val, z_val, nx, ny, nz, file_contents,
    ...
    # === SLIDER CLICKS — handle directly (Y/Z slider fix) ===
    ...
    if triggered_id == 'btn-find-tip' and find_tip_n and find_tip_n > 0:
        try:
            # STEP → STL mesh 변환 후 STL viewer 로직으로 tip 검출
            stl_mesh = step_to_stl_mesh(step_file)
            if stl_mesh is None:
                raise ValueError("Failed to convert STEP to STL mesh")
            tip_points = find_tip_points(stl_mesh, tol=tol_val_input if tol_val_input else 0.5)
        except Exception as e:
            print(f"FindTip error: {e}")
            tip_points = []
    ...
    if triggered_id == 'btn-reset' and reset_n and reset_n > 0:
        ...
        fig3d = build_3d_figure(info['mesh'], 'x', cx, tip_points=_tip_points_store if _tip_points_store else None)
        bx, by, bz = 1, 0, 0
        xs, ys, zs = btn_active(), btn_inactive(), btn_inactive()
        return (fig2d, fig3d,
