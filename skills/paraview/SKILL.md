...
3. Run with `/opt/paraview/bin/pvpython` — **must set `DISPLAY` env var** (`DISPLAY=:0` or from `os.environ`)
4. Save result as `.pvsm` state file in case directory
5. Launch ParaView GUI: `/opt/paraview/bin/paraview --state=<state.pvsm>` with `DISPLAY` env set
   - Or use `launch.py` script which auto-detects `.pvsm` and launches ParaView
6. Keep ParaView GUI open for user inspection

### Display Environment
ParaView GUI needs a display. In headless environments, set DISPLAY before launching:
```bash
export DISPLAY=:0
/opt/paraview/bin/paraview --state=case.pvsm
```
`launch.py` auto-reads `DISPLAY` from environment (default `:0`).
...
## plot_pressure_wall.py (updated 2026-08-17)
Automatically finds wall-type patches from `constant/polyMesh/boundary` file via regex:
```python
matches = re.findall(r'(\S+)\s*\{\s*\n\t*\s*type\s+wall', content, re.MULTILINE)
```
Boundary file에서 찾은 patch 이름 → ParaView용 `patch/name` prefix 자동 추가.
Usage: `pvpython plot_pressure_wall.py <case_dir>` — no hardcoded paths.

## slice_y0_Umag.py (updated 2026-08-17)
Slice at y=0 with velocity magnitude. Now accepts `<case_dir>` as argument instead of hardcoded path.
...
| Script | Description |
|--------|-------------|
| `launch.py` | Run pvpython script + auto-launch ParaView GUI with .pvsm (supports DISPLAY env) |
| `list_patches.py` | List available patches in case |
| `load_case.py` | Load OpenFOAM case (decomposes detection) |
| `plot_patch.py` | Plot field on patch |
| `plot_pressure_wall.py` | Find wall patch from boundary → plot pressure on it |
...
| `wall_p.py` | Wall pressure visualization (hardcoded path) |
...
| `plot_mesh_surface.py` | Mesh surface visualization |
| `plot_surface_mesh.py` | STL + OpenFOAM mesh combined |
| `plot_surface_mesh_offscreen.py` | Offscreen mesh render to PNG |
