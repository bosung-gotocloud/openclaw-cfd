...
def run_calculate():
    ...
    # Size: 0.5*farBox dx x 2*STEP_yl x 2*STEP_zl
    refine_dx = 0.5 * cube_dx
    refine_dy = 2 * yl
    refine_dz = 2 * zl
    ...
    # Check if JSON already has user-modified values
    args_file = os.path.join(base_dir, f"{base_name}_mesh_args.json")
    if os.path.exists(args_file):
        with open(args_file, 'r') as f:
            existing_args = json.load(f)
            # Preserve user modifications if they exist
    ...
    derived_rows = [
        ["T (BL total)", f"{T:.6f} m", "h1*(growth^layers-1)/(growth-1)"],
        ...
        ["max_size (=max(cube_dx,cube_dy,cube_dz)/50)", f"{max_size:.6f} m", "= max(cube_dx,cube_dy,cube_dz)/50"],
    ...
    summary_rows = [
        ...
        # Refine Box
        ["refine_dx", f"{refine_dx:.4f} m", "Wake capture X size (0.5 x far_box_dx)"],
