...
- upstreamPoint: diskCenter + diskDir×0.01×radius (upstream: diskDir direction, incoming velocity 측정용)
...
def main():
    ...
    # upstreamPoint: disk center + diskDir * epsilon (upstream: diskDir direction, incoming velocity 측정)
    epsilon = adm_radius * 0.01  # 반지름의 1% (disk 바로 바깥)
    upstream_x = adm_center_x + diskDir_x * epsilon
    upstream_y = adm_center_y + diskDir_y * epsilon
    upstream_z = adm_center_z + diskDir_z * epsilon
    ...
    print("    (disk center + diskDir × {:.6f})  [upstream: diskDir direction]".format(epsilon))
    print("  epsilon: {:.6f} m (radius × 0.01)".format(epsilon))
