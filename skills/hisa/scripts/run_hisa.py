...
def run_parallel_hisa(case_dir, n_procs):
    """HiSA parallel 실행: decomposePar → mpirun hisa -parallel (detach) → reconstructPar → processor* 삭제.

    2026-09-10: mpirun을 setsid + nohup으로 detach, poll loop에서 대기.
    - openclaw/python 세션 종료 시에도 mpirun이 생존 (새 session, PID 1에 재부착)
    - stdout/stderr → log.hisa 직접 redirect (pipe 없음 → SIGKILL/OOM 방지)
    - poll loop에서 프로세스 종료 후 exit code 확인
    """
    import time

    print(f"\n  === Parallel HiSA ({n_procs} procs) ===")

    # 1. decomposePar (foreground, 빠름)
    ...
    # 2. parallel hisa — setsid+nohup detach, poll loop 대기
    print(f"  [2/4] mpirun hisa -parallel ({n_procs} procs)...")
    case_dir_abs = os.path.abspath(case_dir)
    # setsid: 새 session 생성 (부모 세션과 독립)
    # nohup: SIGHIGN/SIGHUP 무시
    # log.hisa: stdout/stderr redirect (pipe 없음)
    cmd = (
    ...
    # poll loop: 프로세스 종료까지 대기 (1초마다 체크)
    poll_interval = 1  # seconds
    last_poll = time.time()
    while proc.poll() is None:
        time.sleep(poll_interval)
        elapsed = time.time() - last_poll
        if elapsed >= 300:  # 5분마다 중간 보고
            # log.hisa의 마지막 3줄 출력
            try:
                with open(os.path.join(case_dir, 'log.hisa'), 'r') as f:
                    lines = f.readlines()
                tail = '\n'.join(lines[-3:]) if len(lines) >= 3 else ''.join(lines)
                print(f"  [HiSA] Running... (last 3 lines)\n{tail}")
            except (FileNotFoundError, IOError):
                print(f"  [HiSA] Running... (log.hisa not yet created)")
            last_poll = time.time()

    ret = proc.returncode
    print(f"  [2/4] mpirun hisa -parallel 완료 (exit={ret})")

    # 3. reconstructPar (foreground, 빠름)
    if ret == 0:
        ...
        print(f"  [3/4] reconstructPar skip (hisa failed)")
