#!/usr/bin/env python3
"""Rewrite main() in vlm-tip-detect.py: base64 VLM + scaling + proper storage."""

import re, base64, io, shutil

# Step 1: reset to backup
shutil.copy('/home/bosung/.openclaw/workspace/skills/backups/2026-08-11b/vlm-tip-detect.py',
            '/home/bosung/.openclaw/workspace/skills/tip-detect/vlm-tip-detect.py')

fpath = '/home/bosung/.openclaw/workspace/skills/tip-detect/vlm-tip-detect.py'
with open(fpath, 'r') as f:
    content = f.read()

# ========== 1. VL_PROMPT = new format description ==========
old_vlp = 'VL_PROMPT = \'Find sharp pointed tips (wing tips, tail tips, canard tips, etc.) and return their pixel coordinates. Format: [{"tip_id": N, "x": pixelX, "y": pixelY}]\'\'
new_vlp = "VL_PROMPT = 'Find sharp pointed tips (wing wings, tail tips, canard tips, etc.) in this image. Return JSON: {image_width, image_height, tips: [{tip_id, x1, y1, x2, y2}]}'"
content = content.replace(old_vlp, new_vlp)
...
new_get = '''def get_vlm(image_path, timeout_sec=60):
    ...
    try:
        ...
        mtype = 'image/png'
        # Prompt: ask VLM to return its image dimensions too
        prompt = (
...
# ========== 4. Replace main VLM section (the whole loop body for VLM) ==========
# Find the main() VLM section and replace it entirely
# It starts at "raw = get_vlm(op)" and ends before the next "if not any" or main logic

old_main = '''        raw = get_vlm(op)
        if raw:
...
new_main = '''        raw = get_vlm(op)
        ...
        if tips_vlm:
            ...
            # Scale VLM bbox to actual image pixels, then draw + convert to CAD
            actual_w = 1100; actual_h = 1100  # actual image pixel size
            scale_x = actual_w / vlm_w if vlm_w > 0 else 1.0
            scale_y = actual_h / vlm_h if vlm_h > 0 else 1.0
            tips = []
            for tip in tips_vlm:
                # Scale VLM's bbox from VLM's image size to actual image size
