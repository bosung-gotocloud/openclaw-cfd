---
name: salome-elliptic-cone-shock-refine
description: SALOME Netgen-based mesh generation with **elliptic** far-field/refine boxes, shock-wave point-based local sizing and tip-wake line refinement. STEP import → Netgen mesh → OpenFOAM case. No snappyHexMesh.
---

# salome-elliptic-tip-shock-refine Skill (FINAL — 2026-09-12)

**원본 `salome-tip-shock-refine`에서 Far-field/Refine box를 Cube → Ellipsoid로 변경한 버전입니다. 스크립트와 문서의 나머지 내용은 원본과 동일합니다.**

## Elliptic Box Changes (vs tip-shock-refine)

| Feature | tip-shock-refine (original) | elliptic version |
|---------|---------------------------|------------------|
| Far-field domain | `MakeBoxDXDYDZ` → `MakeTranslation` | **`MakeEllipse` + `MakeFaceWires` + `MakeCutList` + `MakeRevolution(OX, 360°)` → Solid Ellipsoid** |
| Refine region | Rectangular box (`refine_tx,ty,tz`) | **Same revolution pattern → Solid Ellipsoid, 25/75 position** |
| Ellipsoid position | center at STEP bbox | **STEP center(0.5xl) = ellipsoid x-extent의 25% 지점 → upstream 25% / downstream 75% (flow = +x)** |

### Ellipsoid Generation (connection error 해결, 첨부 SALOME 패턴)

```python
# 1. Ellipse face (x-y plane, origin center)
ellipse = geompy.MakeEllipse(O, OZ, half_x, half_yz, OX)
ellipse_face = geompy.MakeFaceWires([ellipse], 1)
# 2. x>=0 half-ellipse cut
cut_box = geompy.MakeFaceHW(cube_dx, 2*half_yz, 1)
geompy.TranslateDXDYDZ(cut_box, 0, -half_yz, 0)
half_ellipse = geompy.MakeCutList(ellipse_face, [cut_box])
# 3. Revolution 360° about OX → solid ellipsoid (y=z 대칭)
farfield_ellipsoid = geompy.MakeRevolution(half_ellipse, OX, 360°)
# 4. Position: upstream 25% / downstream 75% of x-extent (flow +x)
extent = 2.0 * half_x
farfield_ellipsoid = geompy.MakeTranslation(farfield_ellipsoid, cx + 0.25*extent, cy, cz)
```

**중요:** `MakeSphere + MakeScaleAlongAxes` 사용 시 curved surface connectivity error로 Netgen meshing 실패. **반드시 revolution 방식 사용.**
**Refine ellipsoid:** 동일한 패턴, `half_x = refine_dx/2`, `half_yz = max(refine_dy, refine_dz)/2`.
| Translate needed? | Yes (tx,ty,tz) | No (centered at STEP bbox center) |
...
calculate_mesh_params.py   (Stage 1)
    ├─ STEP import + bbox
    ├─ Far-field ellipsoid + tool solid
    ├─ Refine ellipsoid size calculation
...
compute_mesh.py            (Stage 2)
    ...
    ├─ RefineEllipsoid (only if `use_refine_ellipsoid=true` in JSON, default OFF)
...
**Wake cone volume refine (2026-09-15)**

| Parameter | Value | Source |
|-----------|-------|--------|
| r1 (tip end) | `surf_size × 2` | Fixed |
| r2 (far end) | `L × tan(min(|AoA|, 45°))` | `aoa_degrees` in JSON |
| L (cone length) | tip → downstream boundary | Calculated |
| refine_size | `r1` (= `surf_size × 2`) | Option A — volume local size |
| AoA range | 0…45° (clamped) | Default 30°, user-editable in JSON |
...
"wake_cone": {
  ...
  "r2": <calculated>,
  "L": <calculated>,
  "refine_size": <calculated>
}
```

**Geometry:** `geompy.MakeCone(r1, r2, L)` per tip → rotate to +x axis → clip to domain → `SetLocalSizeOnShape(cone, refine_size)` — **volume refine, not edge refine**.
...
## Test Execution: myShahed elliptic tip refine (2026-09-12 ✅)

### Setup
- **Case:** `/home/bosung/WinD/0.cfd/01.AI-agent/drone/myShahed/mesh/salome-elliptic-tip-refine/`
- **Geometry:** `myShahed.stp` (drone, xl=2.364m)
- **Tip points:** `myShahed_tip_points.csv` (6 wake lines)
- **Shock CSV:** 없음 (shock refinement off)

### Results
| 항목 | 값 |
|------|-----|
| Far ellipsoid | semi-axes x=5.91m, y=z=2.51m, upstream 25%/downstream 75% |
| Refine ellipsoid | x=5.91m, y=z=2.51m, same 25/75 position |
| Wake lines | 6개, tip_wake_refine_size=0.0341m |
| Mesh | 4,733,534 cells, checkMesh OK |
| Quality | max skewness=7.34 (26 faces), 9535 severely non-orthogonal (tip/wake boundary) |
| case.foam | ✅ (simpleFoam ready) |

### Lessons Learned
1. **`MakeScaleAlongAxes` / `MakeSphere+Scale`** → curved surface connectivity error, Netgen meshing 실패
2. **Revolution 방식 (반드시):** `MakeEllipse → MakeFaceWires → MakeCutList → MakeRevolution(360°)`
3. **Ellipsoid position:** STEP center = x-extent의 25% 지점 (upstream 25% / downstream 75%, flow=+x)
   - `offset = +0.25 * (2 * half_x)` (positive = downstream, x-flow)
4. **Refine ellipsoid:** 동일 패턴, 0.5배 scale (semi-axis 기준)
5. **Test dir 규칙:** 스크립트 수정은 원본 스킬 디렉토리에서만, 테스트 디렉토리는 copy만
