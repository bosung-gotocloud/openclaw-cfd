...
def run_parallel_simplefoam(case_dir, num_procs):
    ...
    cmd = f"bash -c 'source /opt/OpenFOAM/OpenFOAM-v2512/etc/bashrc && cd {case_dir} && decomposePar -force 2>&1 | tee {case_dir}/log.decomposePar'"
    ret = os.system(cmd)
    if ret != 0:
    ...
    # 2026-09-10: setsid+nohup detach — 세션 종료 시에도 mpirun 생존
    cmd = (f"bash -c 'source /opt/OpenFOAM/OpenFOAM-v2512/etc/bashrc && cd {case_dir} && "
           f"setsid nohup mpirun -np {num_procs} --oversubscribe simpleFoam -parallel > {case_dir}/log.simpleFoam 2>&1'")
    ...
    ret = os.system(f"bash -c 'source /opt/OpenFOAM/OpenFOAM-v2512/etc/bashrc && cd {case_dir} && reconstructPar 2>&1 | tee {case_dir}/log.reconstructPar'")
    if ret != 0:
...
def run_single_simplefoam(case_dir):
    ...
    cmd = f"bash -c 'source /opt/OpenFOAM/OpenFOAM-v2512/etc/bashrc && cd {case_dir} && simpleFoam 2>&1 | tee {case_dir}/log.simpleFoam'"
    ret = os.system(cmd)
    if ret != 0:
        print(f"  ERROR: simpleFoam failed (exit {ret})")
        return False
