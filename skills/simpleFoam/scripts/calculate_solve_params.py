...
def calculate_omega(k, L_ref):
    ...
    - The 0.07 factor is for fully-developed pipe flow (L_mix = 0.07*D)
    - External aerodynamics uses chord/characteristic length directly as L
    - L_ref=1.0 m is the standard reference length for airfoil/wing cases
    - OpenFOAM official docs: omega = k^0.5 / (C_mu^0.25 * L) — no 0.07 factor
    """
    Cmu = 0.09
    # Use L_ref directly as the reference length scale (no 0.07 multiplier)
...
def main():
    ...
    # FIXED: L_scale = 1.0 fixed (per hisa workflow)
    L_scale = 1.0
