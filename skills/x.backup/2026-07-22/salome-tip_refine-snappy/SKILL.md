...
# Mesh Quality Options (2026-07-22):
params.SetNbSurfOptSteps(3)        # Surface mesh optimization → skewness ↓
params.SetNbVolOptSteps(3)         # Volume mesh optimization → skewness ↓
params.SetOptimize(1)              # 3D tetrahedra optimization
params.SetSecondOrder(0)           # Linear elements (snappyHexMesh compat)
params.SetGrowthRate(1.3)          # Growth rate limit for quality control
...
|------|------|------|
| **tip_wake_refine_size** | **2 × surf_size** (= xl × 0.02) | Wake line local cell size |
...
## Mesh Sizing Logic (xl-based)

**snappyMesh / salome-tip_refine와 동일한 xl 기반 로직을 사용.**

```
Step 1: max_size   = xl / 5        (배경 메쉬의 가장 큰 셀)
Step 2: surf_size  = xl × 0.01     (표면 cell size = 물체 X 길이의 1%)
Step 3: min_size   = T               (BL 총 두께)
Step 4: refine_size = max_size / 4  (refine box local size)
Step 5: tip_wake_refine_size = 2 × surf_size  (tip wake line local size)
```

> `T` (BL total thickness)는 **sizing에 사용**합니다 (min_size = T).
...
| surf_size | Surface mesh size | **= xl × 0.01** |
| max_size | Max mesh size | = **xl / 5** |
| refine_local_size | Refine box local size | = max_size / 4 |
| tip_wake_refine_size | Wake line local size | = **2 × surf_size** |
...
| `tip_wake_refine_size` | ✗ | = **2 × surf_size** (= xl × 0.02) |
