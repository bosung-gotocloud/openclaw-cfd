...
def run_calculate():
    ...
    h1 = 0.0001       # firstCellHeight (m) — for snappyHexMesh later
    layers = 10        # number of layers (snappyHexMesh)
    growth = 1.2       # expansion ratio (snappyHexMesh)
    fineness = 2       # 2=mod / 3=fine / 4=vfine
    ...
    # Step 1: max_size = xl / 5
    max_size = xl / 5.0

    # Step 2: surf_size = xl * 0.005
    surf_size = xl * 0.005

    # Step 3: min_size = T (BL total thickness)
    min_size = T

    # Step 4: refine_size (refine_local_size) = max_size / 4
    refine_local_size = max_size / 4.0

    # Step 5: tip_wake_refine_size = surf_size (set below after tip CSV check)
    ...
    bl_rows = [
        ...
        ["T (total thickness)", f"{T:.6f} m", "h1*(growth^layers-1)/(growth-1) (= min_size)"],
    ]
    ...
    tip_wake_refine_size = None
    if _deferred_tip_points:
        tip_wake_refine_size = surf_size  # Step 5: tip_wake_refine_size = surf_size
    ...
    summary_rows = [
        ...
        ["tip_wake_refine_size", f"{tip_wake_refine_size:.6f} m" if tip_wake_refine_size else "N/A", "Step 5: surf_size"],
    ]
