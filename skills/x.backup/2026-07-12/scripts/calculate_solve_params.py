#!/usr/bin/env python3
"""calculate_solve_params.py - simpleFoam 계산 파라미터 및 초기값 계산

대화 기반 파라미터 처리:
- mandatory: mesh_path (사용자 필수 제공)
- optional: flow, turbulence, run, reference 파라미터 → default 값 사용
- JSON 생성 → 표시 → 승인
...
def calculate_k(Uinf, I):
    """Turbulent kinetic energy: k = 1.5 * (I * |U|)^2
    
    I = turbulence intensity (e.g., 0.01 for 1%)
    |U| = Uinf magnitude
    """
    return 1.5 * (I * Uinf) ** 2
...
def calculate_omega(k, L_ref):
    """Turbulent frequency: omega = sqrt(k) / (Cmu^0.25 * L)
    
    k = turbulent kinetic energy (from calculate_k)
    L = mixing length (e.g., 1.0e-5)
    Cmu = 0.09
    """
    Cmu = 0.09
...
def auto_estimate_length_scale(mesh_path):
    ...
    far patch의 face area와 point 수를 기반으로 도메인 스케일 계산.
    """
    # polyMesh/boundary에서 far patch의 face count 읽기
    boundary_file = os.path.join(mesh_path, 'boundary')
    if not os.path.isfile(boundary_file):
        return None
    ...
    # far face가 전체 도메인 surface의 대부분 → 도메인 스케일 추정
    # 면적 추정: far_faces * avg_cell_area (체스키 assumption)
    # 더 간단하게: point count에서 bbox 추정
    ...
    # heuristic: far_faces가 많을수록 큰 도메인
    # 면적 근사: far_faces * (L^2 / N_faces_per_L^2)
    # 간단한 추정: n_points의 cube root로 bbox dimension 추정
    # external flow domain: ~10D ~ 20D
    bbox_side_est = max(2.0, (n_points ** 0.15) * 1.5)
    
    # char length = max dimension of bounding box
...
def main():
    ...
    if len(sys.argv) >= 2:
        ...
        if os.path.isfile(input_path):
            ...
            for key in ["flow", "fluid", "turbulence", "run", "reference", "CofR", "boundary"]:
                if key in user_input:
                    params[key].update(user_input[key])
            # Top-level keys (mesh_path, surfaceName, etc)
            for key in ["mesh_path", "surfaceName", "farBC", "surfaceBC"]:
                if key in user_input:
                    params[key] = user_input[key]
        elif os.path.isdir(input_path):
            # Direct directory path — use as mesh_path
            params["mesh_path"] = input_path
            # Also check if polyMesh exists inside
        else:
            # Treat as mesh_path directly
            params["mesh_path"] = input_path
    ...
    if mesh_path and os.path.isdir(mesh_path):
        ...
        if detected:
            surface_patch_auto = detected[0]  # 첫 번째 non-far patch
            # user-provided patch takes priority
    ...
    if mesh_path and os.path.isdir(mesh_path):
        ...
        if os.path.isfile(points_file):
            ...
            # User-set lengthScale takes priority. If still default (0.1), use mesh scale.
            if L_scale == DEFAULTS["turbulence"]["lengthScale"]:
                L_scale = L_mesh
                params['turbulence']['lengthScale'] = L_scale
            print(f"\n=== Mesh Bounding Box ===")
    ...
    I = params['turbulence']['intensity']  # turbulence intensity (e.g., 0.01)
    ...
    Uy = Uinf * cos_aos * sin_aoa
    Uz = Uinf * sin_aos
    freestream_dir = f"[{cos_aos * cos_aoa:.6f} {cos_aos * sin_aoa:.6f} {sin_aos:.6f}]"
    ...
    print(f"  Re (based on L_ref): {Uinf * params['reference']['L_ref'] / nu:.2e}")
    ...
    print(f"\n--- Reference Values ---")
    print(f"  L_ref: {params['reference']['L_ref']} m")
    print(f"  A_ref: {params['reference']['A_ref']} m2")
    print(f"\n--- Hardware ---")
    print(f"  procs: {output['num_procs']}")
    print(f"\nJSON saved to: {output_path}")
    print(f"Edit this file to change any parameters, then run run_simpleFoam.py")
