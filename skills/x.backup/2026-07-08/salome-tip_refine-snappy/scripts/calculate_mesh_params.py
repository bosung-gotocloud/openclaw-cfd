...
def run_calculate():
    ...
    derived_rows = [
        ...
        ["BASE_MINTHICKNESS", f"{base_minthickness:.6f} m", "T x 0.5"],
        ["min_size (=20xT)", f"{min_size:.6f} m", "= 20xT"],
        ["surf_size (=2xmin_size)", f"{surf_size:.6f} m", "= 2xmin_size"],
    ...
    # ------ Tip Wake Lines (after surf_size is known, but BEFORE boolean cut)
    wake_lines = []
    tip_wake_refine_size = None
    if _deferred_tip_points:
    ...
    summary_rows = [
        ...
        ["BASE_MINTHICKNESS", f"{base_minthickness:.6f} m", "T x 0.5"],
        ["min_size", f"{min_size:.6f} m", "20xT"],
        ["surf_size", f"{surf_size:.6f} m", "2xmin_size"],
        ["refine_local_size", f"{refine_local_size:.6f} m", "= max_size / 4"],
        ...
        ["refine_dy", f"{refine_dy:.4f} m", "Wake capture Y size (2xSTEP yl)"],
        ["refine_dz", f"{refine_dz:.4f} m", "Wake capture Z size (2xSTEP zl)"],
        ...
        ["tip_wake_refine_size", f"{tip_wake_refine_size:.6f} m" if tip_wake_refine_size else "N/A", "Wake line local size"],
    ]
