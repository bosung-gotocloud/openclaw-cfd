...
def run_compute():
    ...
    template_dir = os.path.expanduser("~/.openclaw/workspace/skills/salome-tip_refine-snappy/case_template")
    ...
    mpirt_run = ['mpirun', '--oversubscribe', '-np', '8', 'snappyHexMesh', '-case', case_path]
    print(f"  Running: {' '.join(mpirt_run)}")
    r2 = subprocess.run(mpirt_run, capture_output=True, text=True, timeout=3600, env=env)
    if r2.returncode == 0:
        print("  snappyHexMesh: OK")
    else:
        print(f"  snappyHexMesh: FAIL (rc={r2.returncode})")
        # Try serial fallback
