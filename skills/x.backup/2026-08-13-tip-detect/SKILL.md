...
# vlm-tip-detect.py — VLM-Based STEP CAD Tip Detection
...
### 프롬프트
```
Find ALL tips, sharp corners, wing tips, tail tips, canard tips — sharp concave points. 
...
| 파일 | 형식 | 설명 |
|------|------|------|
| `{name}_tip_points.csv` | CSV (헤더 없음) | `x,y,z` (mm, 6자리 소수점), 한 줄 = 한 tip |
| `{name}_tip.png` | PNG | Isometric 뷰 + 빨간 tip marker (r=10px) |
...
## `step-find-tip.py`와의 차이점

| 항목 | `vlm-tip-detect.py` | `step-find-tip.py` |
|------|-------------------|-------------------|
| **검출 방식** | **VLM 시각 기반** (이미지 → 픽셀 좌표) | **CAD native edges 기반** (기하 알고리즘) |
| **검출 대상** | wing tip, tail tip, canard, sharp concave points 등 모든 돌출부 | wing tip, tail tip, sharp corner (convexity filter 포함) |
| **이미지 렌더링** | XZ/YZ/XY 3 orthographic views (STL face-filled) | XZ/YZ/XY 3 orthographic views (uniform mesh) |
| **좌표 역산** | pixel → scale 보정 → fit_scale+offset → CAD(mm) | CAD edge projection → sharp corner detection (tan angle > 60°) |
| **VLM 사용** | 필수 (qwen3.6:35b, pixel 기반) | 필요 없음 |
| **프롬프트** | "Find ALL tips, sharp corners, wing tips, tail tips, canard tips — sharp concave points." | N/A |
| **스케일 보정** | VLM이 반환한 image_width/height 기반 동적 스케일링 | N/A |
| **Silhouette** | trimesh STL face-filled (back-face culling + gray fill) | STEP native edges → uniform mesh → projection → 외곽선 추출 |
| **Sharp corner** | VLM 식별 (프롬프트 기반) | tangent angle > 60° (cos < 0.5) + endpoints |
| **Convexity** | N/A | face normal outward dot > 0.3 |
| **Tip classification** | N/A (tip_id만) | wing_tip / tail_tip / other |
| **Triangulation** | 3-view hints clustering → 3D 복원 → 50mm dedup → Y-Z dedup + Max-X | Y-Z dedup → max-X filter → -X 필터 (nose 제외) |
| **Nose 필터링** | VLM 프롬프트에 "sharp concave points"만 (nose 직접 제외 없음) | bbox center보다 X < 0인 점 제외 |
| **Dedup tol** | `max(xl,yl,zl) × 0.05` (3D 기준 dx,dy,dz 모두) | `max(bbox_span × 1%, 5mm)` (silhouette 기반) |
| **출력** | CSV + ISO PNG + annotated PNGs | CSV + canvas PNG (800×800) |
| **장점** | 복잡한 형상에서 VLM 식별력 우수, sharp concave points 검출 가능 | VLM 의존도 없음, CAD native 정확도 |
| **단점** | VLM 응답 불안정, image_size mismatch 이슈, 느림 | silhouette로 검출 안 되는 small protrusion 놓침 |
| **권장** | **기본 사용** (시각적 tip이 중요할 때) | **fallback** (VLM 없이 CAD만 사용할 때) |
