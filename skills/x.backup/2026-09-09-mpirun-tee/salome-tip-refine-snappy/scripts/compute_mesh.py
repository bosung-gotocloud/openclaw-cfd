...
def run_compute_mesh():
    ...
    derived_rows = [
        ...
        ["min_size", f"{min_size:.6f} m", "2xT (BL-based)"],
    ...
    # Find the smallest face bbox diagonal from the STEP file
    ...
    for face in all_shape_faces:
        bb = geompy.BoundingBox(face)
        dx = bb[1] - bb[0]
        dy = bb[3] - bb[2]
        dz = bb[5] - bb[4]
        import math as _math
        diag = _math.sqrt(dx*dx + dy*dy + dz*dz)
        if diag < smallest_face_diag:
            smallest_face_diag = diag
    print(f"  Smallest face bbox diagonal: {smallest_face_diag:.8f} m")
    ...
    # Adjust min_size if it's larger than the smallest face's bbox diagonal
    # (Netgen cannot mesh faces smaller than min_size)
    if smallest_face_diag > 0:
        if min_size > smallest_face_diag:
            old_min_size = min_size
            min_size = smallest_face_diag
            args['min_size'] = min_size
            print(f"  [min_size] Adjusted: {old_min_size:.6f} m → {min_size:.8f} m")
            print(f"    Reason: smallest face bbox diagonal ({smallest_face_diag:.6f} m) < original min_size")
        else:
            print(f"  [min_size] OK: {min_size:.6f} m < smallest face diag ({smallest_face_diag:.6f} m)")
    ...
    # params.SetSecondOrder(0)           # Linear elements (snappyHexMesh compat)
...
""" % (num_procs, num_procs))
        ...
        decomp_cmd = f"bash -c 'source /opt/OpenFOAM/OpenFOAM-v2512/etc/bashrc 2>/dev/null && cd {case_dir} && decomposePar -force 2>&1 | tee {decomp_log}'"
        ...
        run_cmd = f"bash -c 'source /opt/OpenFOAM/OpenFOAM-v2512/etc/bashrc 2>/dev/null && cd {case_dir} && mpirun --oversubscribe -np {num_procs} snappyHexMesh -parallel 2>&1 | tee {log_file}'"
