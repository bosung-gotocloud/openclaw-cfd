# snappyMesh-tip_refine 스킬 — 전체 설명
...
### Step 3: `snappy_mesh.py` — snappyHexMesh 전체 실행

```bash
python3 snappy_mesh.py LC62-50H_mesh_args.json
```
...
Wake box 크기:
 ...
 Z 방향: tip_z ± (surf_size × 5) (총 폭 = 10×surf_size)

wake_half_w = surf_size × 5
min = (tip_x, tip_y - wake_half_w, tip_z - wake_half_w)
max = (refine_cx + refine_dx/2, tip_y + wake_half_w, tip_z + wake_half_w)
```
...
날개 팁에서 떨어지는 와류는 매우 얇고 긴 구조입니다. wake line refinement은 이 와류가 지나는 경로를 높은 해상도로 메쉬링하여 와류 구조를 정확하게 포착합니다. Y/Z로 10×surf_size만큼의 폭을 주는 것은 와류의 확산을 충분히 커버하기 위함입니다.
...
| 항목 | snappyMesh | snappyMesh-tip_refine |
|---|---|------|
...
│ └── snappy_mesh.py ← Stage 3: snappyHexMesh 전체 workflow
└── assets/
